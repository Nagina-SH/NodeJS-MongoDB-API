...
MongoClient.connect('mongodb://localhost/node-mongodb', (err, db) => {
	if(err){
		console.log(err);
		return console.log('Unable to connect to node-mongoDB');
	}
	console.log('connect to node-mongoDB');
});
...
MongoClient.connect('mongodb://localhost/nodejsdb', (err, db) => {
	...
	db.collection('employess_work').insert({
		text: 'Work done by shailendra',
		completed: 2,
